...
public class TicketMasterClient {
	...
	//get request from ticketmaster return info we need and convert JSONobject to string
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			connection.connect(); // response include in connection
			...
			int responseCode = connection.getResponseCode(); // check is 200?
			...
			// IO stream Read from connection(input)
			...
			BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream())); // read
																											...
																											// bufferReader
			String line;
			// Read line by line until read to the null
			while ((line = reader.readLine()) != null) {
				// append line into responseBody
				responseBody.append(line);
			}
			reader.close();
		} catch (MalformedURLException e) { // URL connection
			e.printStackTrace();
		} catch (IOException e) { // bufferReader
			e.printStackTrace();
		}
		// convert stringBuilder to string, string to JSON obj
		try {
			// Extract events array only.
			JSONObject obj = new JSONObject(responseBody.toString());
			// error handle:Format is wrong
	...
	private String getImageUrl(JSONObject event) throws JSONException {
		// cchek edge case, JSONobject is already check in api event is not null
